...
flu <- flu_cases %>%
  full_join(flu_pop, by = c("county","demographic_value")) %>%
  mutate(week_beginning = floor_date(dt_diagnosis,"week"))
...
vax <- source_vax_CA %>%
  ...
  mutate(dt_admin = as.Date(dt_admin, format = "%Y-%m-%d"),
    demographic_value = case_when(demographic_value == "Under 5" ~ "0-17",
                              ...
                              #  demographic_value == "Unknown/undifferentiated" ~ "Unknown gender", or as NA since we don't have such data in flu 
  ...
  ungroup() %>%
  mutate(dt_admin = as.Date(dt_admin),
        week_beginning = floor_date(dt_admin, "week"))
